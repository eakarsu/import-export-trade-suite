const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5410';
...
const adminCookie = await login('admin@import-export-trade.local', 'admin123');
const managerCookie = await login('manager@import-export-trade.local', 'manager123');
const analystCookie = await login('analyst@import-export-trade.local', 'analyst123');
